...
class ManagerLaborCost(models.Model):
    _name = 'project.manager.labor.cost'
    _description = 'BÁO CÁO HIỆU QUẢ QUẢN LÝ CHI PHÍ NHÂN CÔNG DỰ ÁN'
    ...
    driff = fields.Float('Trung gian')
    reward = fields.Float('Dự kiến thưởng', compute='compute_amount')
    ...
    def compute_amount(self):
        for r in self:
            ...
            r.total = a
            if a >0:
                r.reward = min(a,r.driff)
            else: r.reward = min(0,r.driff)
