    ...
    def google_drive_upload(self, rec, file_path, bkp_file):
        ...
        if rec.is_upload:
            ...
            if r.status_code == 200:
                ...
                #     email_to += record + ','
                #
                # notification_template = self.env['ir.model.data'].sudo().get_object('backup_upload_google_drive',
                #                                                                     'email_google_drive_upload')
                # values = notification_template.generate_email(self.id, ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'])
