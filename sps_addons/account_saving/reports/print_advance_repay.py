...
class PrintAdvanceRepay(models.Model):
    _inherit = 'account.move'
    _description = 'Đề nghị hoàn chứng từ'

    def get_job_repay_description(self):
        self.ensure_one()
        name_user = self.env.user.name
        dir_path = os.path.dirname(os.path.realpath(__file__))
        wb = openpyxl.load_workbook(dir_path + '%s..%stemplates%shoan_chung_tu.xlsx' % (os.sep, os.sep, os.sep))
        ws = wb['Sheet1']
        sql = f'''
                SELECT
                    COALESCE(ppr.name, so.name) ma_duan,
                    so.work_content ten_duan,
                    rp.NAME khach_hang,
                    ...
                    rp1.NAME ten_ncc,
	                concat(rp1.street, case when rp1.street is not null and rp1.city is not null then '-' else '' end ,rp1.city, case when rp1.city is not null and rcs.name is not null then '-' else '' end ,rcs.name) dia_chi,
                    rpb.acc_number stk,
                    rp1.NAME nguoi_thu_huong,
                    rb.NAME ngan_hang,
                ...
                FROM
                    ...
                    LEFT JOIN project_project ppr ON ppr.ID = aml.x_project_id
                ...
                ORDER BY ct.cost_group , COALESCE(ppr.name, so.name)
            '''
        ...
        dict_item = {}
        for rec in recs:
            key = (rec['nhom'] or '') + (rec['ma_duan'] or '')
        ...
        # ws.cell(row, 7).value = 'Số tài khoản:'
        # ws.cell(row, 8).value = recs[0].get('stk', None)
        ...
        # ws.cell(row, 7).value = 'Người thụ hưởng:'
        # ws.cell(row, 8).value = recs[0].get('nguoi_thu_huong', None)

        row += 1
        ws.cell(row, 2).value, ws.cell(row, 2).style = 'Địa chỉ',highlight12
        ws.cell(row, 3).value, ws.cell(row, 3).style = recs[0].get('dia_chi', None),highlight12
        # ws.cell(row, 7).value = 'Ngân hàng'
        # ws.cell(row, 8).value = recs[0].get('ngan_hang', None)
        ...
        attachment_id = self.env['ir.attachment'].create({
            'name': 'Đề nghị hoàn chứng từ.xlsx',
